...
//
//  Created by Jose Flores on 6/24/19.
...
class CompleteViewController: UIViewController {
    ...
    @IBOutlet weak var fraseText: UITextView!
    @IBOutlet weak var imagenPregunta: UIImageView!
    @IBOutlet var opciones: [UIButton]!
    @IBOutlet weak var imagenLogueo: UIImageView!
    @IBOutlet weak var nombreLogueo: UILabel!
    ...
    override func viewDidLoad() {
        ...
        let full : String = quest.descrip!
        let fullNameArr : [String] = full.components(separatedBy: "___")
        
        // And then to access the individual words:
        
        var first : String = fullNameArr[0]
        var last : String = ""
        ...
        let stringop = "\(first) (⬛️⬛️⬛️⬛️⬛️) \(last)"
        
        let attributedString = NSMutableAttributedString.init(string: stringop)
        let range = (stringop as NSString).range(of: "(⬛️⬛️⬛️⬛️⬛️)")
        
        
        attributedString.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor.darkGray, range: range)
      
        fraseText.attributedText = attributedString
        fraseText.isUserInteractionEnabled = true
        fraseText.isEditable = false
        fraseText.font = UIFont.systemFont(ofSize: 24, weight: .bold)
        
        //fraseText.text = quest.descrip
        //fraseText.text = "\(first) (___) \(last)"
        let datan = quest.photo
        if let imagen = UIImage(data: datan!) {
            imagenPregunta.image = imagen
        }
    ...
    @IBAction func atrasBtn(segue: UIStoryboardSegue) {
        //if recibir = segue.source as?
